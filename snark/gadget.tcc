...
sudoku_encryption_key<FieldT>::sudoku_encryption_key(protoboard<FieldT> &pb,
                                            //    unsigned int dimension,
                                               ...
                                               ) : gadget<FieldT>(pb, FMT(this->annotation_prefix, " sudoku_closure_gadget")),
                                                   seed_key(seed_key), len(len)// dimension(dimension)
{
...
//                                                ) : gadget<FieldT>(pb, FMT(this->annotation_prefix, " sudoku_cell_gadget")),
...
template<typename FieldT>
sudoku_gadget<FieldT>::sudoku_gadget(protoboard<FieldT> &pb, unsigned int solution_len) :
        gadget<FieldT>(pb, FMT(this->annotation_prefix, " l_gadget"))
{
    // std::cout << "in sudoku_gadget" << std::endl;
    // dimension = n * n;
    len = solution_len;
    ...
    // 不需要puzzle，因此len*8不需要*2
    const size_t input_size_in_bits = len * 8 + /* H(K) */ 256;
    {
        const size_t input_size_in_field_elements = div_ceil(input_size_in_bits, FieldT::capacity());
        // std::cout << "input_size_in_field_elements: " << input_size_in_field_elements << std::endl;
        // 输入，有限域元素
        // std::cout << "allocating input_as_field_elements" << std::endl;
    ...
    solution_values.resize(len);
    // solution_numbers.resize(dimension*dimension);
    encrypted_solution.resize(len); // 这里出错了！！！encrypted_solution也需要resize
    ...
    for (unsigned int i = 0; i < len; i++) {
        ...
        // std::cout << "allocating solution_values[" << i << "]... " << std::endl;
    ...
    // for (unsigned int i = 0; i < (dimension*dimension); i++) {
    for (unsigned int i = 0; i < len; i++) {
        // std::cout << "start allocating encrypted_solution[" << i << "]... " << std::endl;
        encrypted_solution[i].allocate(pb, 8, "encrypted_solution[i]");
        // // std::cout << "allocating encrypted_solution[" << i << "]... " << std::endl;
    ...
    // std::cout << "resetting seed_key" << std::endl;
    ...
    key.reset(new sudoku_encryption_key<FieldT>(pb, len, seed_key_cropped));
...
{
    // std::cout << "in sudoku_gadget::generate_r1cs_constraints" << std::endl;
    // std::cout << "len: " << len << std::endl;
    // for (unsigned int i = 0; i < (dimension*dimension); i++) {
    for (unsigned int i = 0; i < len; i++) {
        for (unsigned int j = 0; j < 8; j++) {
            // ensure bitness
            // std::cout << "calling generate_boolean_r1cs_constraint for solution_values[" << i << "][" << j << "]..." << std::endl;// << solution_values[i][j] << std::endl;
    ...
    //}
    // std::cout << "calling seed_key->generate_r1cs_constraints" << std::endl;
    ...
    )
{
    ...
    assert(input_solution_values.size() == len);
    // assert(input_encrypted_solution.size() == dimension*dimension);
    assert(input_encrypted_solution.size() == len);
